...
function Pending(props : Props) : JSX.Element{
    ...
    return (
        ...
        <section id="displayPendingId">
            {Object.keys(pendingOrder).map( (key : string) : JSX.Element => {
                return ( props.table == "all" || props.allOrd[pendingOrder[Number(key)]]["no"].toString() == props.table.toString()) ?
                <DisplayOrderCard key={key} id={pendingOrder[Number(key)]} allOrd={props.allOrd} buttonDisplayValue={"Move To Processing"} items= {props.items}/> : <></>;
            })}
        </section>
        </>
    );
